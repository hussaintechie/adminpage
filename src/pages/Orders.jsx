...
export default function Orders() {
  ...
  const handleOutForDelivery = async (e, orderId, status) => {
    ...
    if (status === "cancelled") {
      toast.error("Cancelled order cannot be sent for delivery.");
      return;
    }
  ...
  const markcancel = async (e, orderId, status) => {
    e.stopPropagation();

    if (status === "delivered") {
      toast.error("Cannot cancel after  Delivered");
      return;
    }
    ...
    try {
      const res = await markcancelAPI(orderId);
      if (res.data.status === 1) {
        fetchorders();
      } else {
        toast.error(res.data.message);
    ...
    return (
      <div className="max-w-6xl mx-auto pb-24 md:pb-10 min-h-screen px-4 md:px-8 py-6 animate-in slide-in-from-right duration-300">
        <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 -mx-4 px-4 py-3 flex items-center justify-between md:hidden shadow-sm">
          <button
            onClick={() => setSelectedOrderId(null)}
            className="flex items-center gap-1 text-slate-700 font-semibold active:text-slate-900"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4 md:mt-0">
            ...
            <div className="bg-linear-to-br from-white to-slate-50 p-6 rounded-2xl shadow-lg">
              <div>
                ...
                <p className="text-sm text-slate-500 flex items-center gap-2">
                  <Calendar size={14} /> {orderDetails.date} {orderDetails.time}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center text-sm text-slate-600">
                  ...
                  <span className="font-semibold text-slate-800">
                    ₹{orderDetails.summary.handlingFee}
                  </span>
                </div>
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 space-y-5 h-fit">
            <div>
              <h3 className="font-bold text-slate-800 border-b border-slate-100 pb-3 mb-3">
                Customer
              </h3>
              <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex justify-between items-end">
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center w-10 h-10 bg-slate-800 text-white rounded-xl shadow-lg active:scale-90 transition-transform"
          >
            <Filter size={18} />
          </button>
        </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border border-gray-200 rounded-lg p-2 text-sm text-slate-700 outline-none focus:border-emerald-500 bg-slate-50"
                    >
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full border border-gray-200 rounded-lg p-2 text-sm text-slate-700 outline-none focus:border-emerald-500"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full border border-gray-200 rounded-lg p-2 text-sm text-slate-700 outline-none focus:border-emerald-500 bg-white"
                          >
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 text-sm text-red-500 hover:bg-red-50 py-2 rounded-lg transition-colors"
                    >
      ...
      <div className="md:hidden space-y-4">
        {currentItems.map((order) => (
          <div
            ...
            className="group relative bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden active:scale-[0.98] transition-all duration-200"
          >
            <div
              className={`absolute left-0 top-0 bottom-0 w-1.5 ${getStatusColor(order.status)}`}
            ></div>
            <div className="p-4 pl-5">
              ...
              <div className="flex items-start gap-3 mb-3">
                <div className="w-12 h-12 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400 shrink-0 border border-slate-200">
                  <ImageIcon size={20} />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2 bg-slate-50 text-slate-600 text-xs font-bold rounded-lg border border-slate-100 hover:bg-slate-100"
                >
                  <FileText size={14} /> Invoice
                </button>
                <button
                  onClick={(e) =>
                    handleOutForDelivery(e, order.id, order.status)
                  }
                  disabled={
                ...
                <button
                  onClick={(e) => markcancel(e, order.id, order.status)}
                  disabled={
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {STATUS_OPTIONS.map((status) => (
                    <button
                      ...
                      className={`p-3 rounded-xl text-sm font-medium transition-all ${statusFilter === status ? "bg-slate-800 text-white shadow-md" : "bg-slate-50 text-slate-600"} ${loading ? "opacity-50 pointer-events-none" : ""}`}
                    >
                      {status}
                    </button>
                  ))}
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  disabled={loading}
                  onClick={() => setIsFilterOpen(false)}
                  className="  py-3 text-white font-bold bg-emerald-600 rounded-xl shadow-lg shadow-emerald-200 disabled:opacity-50"
                >
                  Apply Filters
                </button>
              </div>
